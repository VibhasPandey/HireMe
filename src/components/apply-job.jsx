...
const schema = z.object({
  experience:z.number().min(0,{message:"Experience must atleast be 0 "}).int(),
  ...
  resume:z.any().refine(file=>file[0] && 
    (file[0].type === "application/pdf" || file[0].type === "application/msword"), {message : "Only PDF or Word documents are allowed"}),
})
...
const ApplyJobDrawer = ({user,job,applied=false,fetchJob }) => {
  ...
  <DrawerTrigger asChild>
    ...
    disabled={!job?.isOpen || applied }>
        {job?.isOpen ? (applied ? "Applied" : "Apply") : "Hiring Closed"}</Button>
  </DrawerTrigger>
  <DrawerContent>
    <DrawerHeader>
    ...
    type="file"
    accept=".pdf,.doc,.docx"
    className="flex-1 file:text-gray-500 my-2 mx-4"
    {...register("resume")}/>
